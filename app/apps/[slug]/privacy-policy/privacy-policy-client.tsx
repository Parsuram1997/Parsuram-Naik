...
import { ChevronLeft, Link as LinkIcon, Check, FileText, Trash2, Info } from "lucide-react";
import { cn } from "@/lib/utils";

export function PrivacyPolicyClient({ app }: { app: AppData }) {
  ...
  useEffect(() => {
    if (!app.privacyPolicy) return;

    const observers = app.privacyPolicy.sections.map((section) => {
  ...
  if (!app.privacyPolicy) return null;
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-12 glass print:bg-transparent print:border-none print:p-0">
              ...
              <div className="mb-12 border-b border-white/10 pb-8 print:border-black/20">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 text-muted-foreground print:text-gray-600">
                  ...
                  <p>Last Updated: <strong className="text-white print:text-black">{app.privacyPolicy.lastUpdated}</strong></p>
                </div>
              </div>
